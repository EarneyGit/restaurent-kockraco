...
export default function BusinessOffersPage() {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const response: BusinessOfferResponse =
        await businessOfferService.getBusinessOffers(currentPage, 20);
                    ...
                    <tr key={offer._id} className="border-b">
                      ...
                      <td className="py-3">
                        <div className="max-w-xs truncate">
                          {businessOfferService.truncateContent(
                            offer.content,
                            50
                          )}
                        </div>
                      ...
                      <td className="py-3">
                        {offer.startDate
                          ? businessOfferService.formatDate(offer.startDate)
                          : "-"}
                      </td>
                      <td className="py-3">
                        {offer.endDate
                          ? businessOfferService.formatDate(offer.endDate)
                          : "-"}
                      </td>
                      <td className="py-3">{offer.displayOrder}</td>
                      <td className="py-3">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${businessOfferService.getOfferStatusBadgeColor(
                            offer.isActive
                          )}`}
                        >
