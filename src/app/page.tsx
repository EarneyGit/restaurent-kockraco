  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const hourlyData = Array.from({ length: 24 }, (_, hour) => {
      ...
      const sales = Math.floor(
        baseSales + Math.random() * baseSales * 0.4 - baseSales * 0.2
      );
      const orders = Math.floor(
        baseOrders + Math.random() * baseOrders * 0.4 - baseOrders * 0.2
      );
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const currentTotalSales = filteredData.reduce(
      (sum, day) => sum + day.totalSales,
      0
    );
    const currentTotalOrders = filteredData.reduce(
    ...
    const previousTotalSales = previousData.reduce(
      (sum, day) => sum + day.totalSales,
      0
    );
    ...
    const hourlyAverages = Array.from({ length: 24 }, (_, hour) => {
      const salesSum = filteredData.reduce(
        (sum, day) =>
          sum + (day.hourlyData.find((h) => h.hour === hour)?.sales || 0),
        0
      );
      const ordersSum = filteredData.reduce(
        (sum, day) =>
          sum + (day.hourlyData.find((h) => h.hour === hour)?.orders || 0),
        0
      );
      ...
      comparisonStats: {
        ...
        ordersChange:
          previousTotalOrders > 0
            ? ((currentTotalOrders - previousTotalOrders) /
                previousTotalOrders) *
              100
            : 0,
        avgOrderChange:
