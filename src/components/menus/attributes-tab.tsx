...
export function AttributesTab({
  ...
  const [productAllowAddWithoutChoices, setProductAllowAddWithoutChoices] =
    useState(false);

  // Modal states
  const [showCreateAttributeModal, setShowCreateAttributeModal] =
    useState(false);
  ...
  const [showProductSelectionModal, setShowProductSelectionModal] =
    useState(false);
  const [showAttributeOptionsModal, setShowAttributeOptionsModal] =
    useState(false);
  ...
  const [currentOption, setCurrentOption] = useState<AttributeOption | null>(
    null
  );
  const [selectedAttributeForOptions, setSelectedAttributeForOptions] =
    useState<Attribute | null>(null);
  const [copyType, setCopyType] = useState<"from" | "to" | "category">("from");
  const [searchTerm, setSearchTerm] = useState("");

  const [errorMessageForMinAttribute, setErrorMessageForMinAttribute] =
    useState("");
  const [errorMessageForMaxAttribute, setErrorMessageForMaxAttribute] =
    useState("");
  // Form states
  ...
  useEffect(() => {
    const { minAttribute, maxAttribute } = newAttribute;

    if (minAttribute > maxAttribute) {
      setErrorMessageForMaxAttribute(
        "Max attribute cannot be less than min attribute."
      );
    } else if (maxAttribute < minAttribute) {
      setErrorMessageForMaxAttribute(
        "Min attribute cannot be greater than max attribute."
      );
    } else {
      setErrorMessageForMinAttribute("");
      setErrorMessageForMaxAttribute("");
    }
  }, [newAttribute.minAttribute, newAttribute.maxAttribute]);
  ...
  const fetchProductAttributes = async () => {
    ...
    try {
      const response = await api.get(
        `/product-attribute-items/product/${productId}`
      );
  ...
  const fetchProductSettings = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log("📥 Fetched allowAddWithoutChoices:", {
          raw: data.data.allowAddWithoutChoices,
          boolean: allowValue,
        });
  ...
  const updateProductAllowAddWithoutChoices = async (value: boolean) => {
    ...
    try {
      ...
      const response = await api.put(`/products/${productId}`, {
        allowAddWithoutChoices: value,
      });
      const data = response.data;

      if (data.success) {
        toast.success(
          `Product setting updated: ${
            value ? "Allow" : "Require"
          } add without choices`
        );
  ...
  const handleCreateAttribute = async () => {
    ...
    try {
      ...
      const attributeData = {
        ...
        maxAttribute: newAttribute?.maxAttribute || 0,
      };
      ...
      if (data.success) {
        ...
        const newAttr = {
          ...
          maxAttribute: data.data.maxAttribute,
  ...
  const handleAddExistingAttribute = async (attributeId: string) => {
    try {
      ...
      const existingAttribute = productAttributes.find(
        (pa) => pa.attribute.id === attributeId
      );
  ...
  const handleRemoveAttribute = async (attributeId: string) => {
    if (
      !confirm(
        "Are you sure you want to remove this attribute and all its options from this product?"
      )
    )
      return;
    ...
    try {
      ...
      const attributeToRemove = productAttributes.find(
        (pa) => pa.attribute.id === attributeId
      );
    ...
    try {
      ...
      if (currentOption) {
        ...
        response = await api.put(
          `/product-attribute-items/${currentOption.id}`,
          formData
        );
      } else {
        // Create new option
      ...
      if (data.success) {
        toast.success(
          `Option ${currentOption ? "updated" : "created"} successfully`
        );
  ...
  const handleUpdateHiddenStatus = async (
    optionId: string,
    field: "hiddenForToday" | "fullyHidden",
    value: boolean
  ) => {
    try {
      ...
      if (value) {
        ...
        const otherField =
          field === "hiddenForToday" ? "fullyHidden" : "hiddenForToday";
        updateData[otherField] = false;
      }

      await api.patch(
        `/product-attribute-items/${optionId}/hidden-status`,
        updateData
      );
      ...
      setProductAttributes((prev) =>
        prev.map((productAttribute) => ({
          ...
          options: productAttribute.options.map((option) =>
            option.id === optionId ? { ...option, ...updateData } : option
          ),
        }))
      );
      ...
      toast.success(
        `Option ${
          field === "hiddenForToday" ? "hidden for today" : "fully hidden"
        } status updated`
      );
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col space-y-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label
              htmlFor="productAllowAddWithoutChoices"
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-500 border-2 border-dashed rounded-lg">
          ...
          <p className="text-sm">
            Use the buttons above to create new attributes or add existing ones.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {productAttributes.map((productAttribute) => (
            <div
              key={productAttribute.attribute.id}
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant={
                        productAttribute.attribute.type === "single"
                          ? "default"
                          : "secondary"
                      }
                    >
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleShowAttributeOptions(productAttribute.attribute)
                      }
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleRemoveAttribute(productAttribute.attribute.id)
                      }
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-gray-500">
                      {(productAttribute.options || []).length} options
                    </span>
                  </div>
      ...
      <Dialog
        ...
        onOpenChange={setShowCreateAttributeModal}
      >
              ...
              <>
                <div>
                  ...
                  <Input
                    id="minAttribute"
                    type="number"
                    value={newAttribute.minAttribute}
                    onChange={(e) =>
                      setNewAttribute((prev) => ({
                        ...prev,
                ...
                <div>
                  ...
                  <Input
                    id="maxAttribute"
                    type="number"
                    value={newAttribute.maxAttribute}
                    onChange={(e) =>
                      setNewAttribute((prev) => ({
                        ...prev,
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                ...
                (newAttribute.type === "multiple" &&
                  Boolean(errorMessageForMinAttribute)) ||
                Boolean(errorMessageForMaxAttribute)
              }
            >
              Save
            </Button>
      ...
      <Dialog
        ...
        onOpenChange={setShowAddExistingModal}
      >
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <Input placeholder="Search attributes..." className="pl-10" />
            </div>
            ...
            <div className="max-h-96 overflow-y-auto space-y-2">
              {allAttributes
                .filter(
                  (attr) =>
                    !productAttributes.some((pa) => pa.attribute.id === attr.id)
                )
                .map((attribute) => (
                  <div
                    key={attribute.id}
      ...
      <Dialog
        open={showCopyAttributesModal}
        onOpenChange={setShowCopyAttributesModal}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              >
                <div className="text-left">
                  <div className="font-medium">
                    Copy attributes from '{productName || "this product"}'
                  </div>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowCopyAttributesModal(false)}
            >
              Close
            </Button>
          </DialogFooter>
      ...
      <Dialog
        open={showAttributeOptionsModal}
        onOpenChange={setShowAttributeOptionsModal}
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedAttributeForOptions && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                <Badge
                  variant={
                    selectedAttributeForOptions.type === "single"
                      ? "default"
                      : "secondary"
                  }
                >
              ...
              <div className="border rounded-lg">
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {productAttributes
                        .find(
                          (pa) =>
                            pa.attribute.id === selectedAttributeForOptions.id
                        )
                        ?.options?.map((option) => (
                          <tr
                            key={option.id}
                ...
                {!productAttributes.find(
                  (pa) => pa.attribute.id === selectedAttributeForOptions.id
                )?.options?.length && (
                  <div className="text-center py-8 text-gray-500">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setShowAttributeOptionsModal(false)}
            >
              Close
            </Button>
          </DialogFooter>
      ...
      <Dialog
        open={showAddOptionModal || showEditOptionModal}
        onOpenChange={(open) => {
          if (!open) {
            setShowAddOptionModal(false);
            setShowEditOptionModal(false);
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {currentOption ? "Edit" : "Add"} Attribute Option for:{" "}
              {selectedAttributeForOptions?.name}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewOption((prev) => ({
                    ...prev,
                    calorificValue: parseInt(e.target.value) || 0,
                  }))
                }
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewOption((prev) => ({ ...prev, weight: e.target.value }))
                }
